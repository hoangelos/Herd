...
msg = { 'update_local_data': 'phalliday<===>This is the SECOND UPDATE!'}
manager_client = HerdClusterClient(seeds=['127.0.0.3'])
response = manager_client.request(msg)

gevent.sleep(10)
print "Verify that CLUSTER NODE #1 doesn't get the update"
msg2 = { 'get_local_data': 'phalliday' }
manager_client = HerdClusterClient(seeds=['127.0.0.2'])
response = manager_client.request(msg2)

gevent.sleep(10)
print "Verify that CLUSTER NODE #2 DOES still get the update"
msg2 = { 'get_local_data': 'phalliday' }
manager_client = HerdClusterClient(seeds=['127.0.0.3'])
response = manager_client.request(msg2)
